...
def fit_coeffs_to_target_phase(
    ...
    # Calculate reconstructed field amplitude
    amplitude_fit = np.abs(field_fit)

# Threshold relative to the peak amplitude
    amplitude_cutoff = (
        AMPLITUDE_THRESHOLD * np.max(amplitude_fit)
    )

# True where phase is considered meaningful
    amplitude_mask = amplitude_fit >= amplitude_cutoff

# Calculate errors only in sufficiently bright pixels
    valid_residual = phase_residual[amplitude_mask]

    if valid_residual.size > 0:
        rms_phase_error = float(
            np.sqrt(np.mean(valid_residual ** 2))
        )
        mean_abs_phase_error = float(
            np.mean(np.abs(valid_residual))
        )
    else:
        rms_phase_error = np.nan
        mean_abs_phase_error = np.nan

# Hide low-amplitude phase pixels
    phase_fit_masked = np.where(
        amplitude_mask,
        phase_fit,
        np.nan,
    )

    phase_residual_masked = np.where(
        amplitude_mask,
        phase_residual,
        np.nan,
    )
...
def main() -> None:
    ...
    for index in range(n_test):
        ...
        if index == 0:
            ...
            example_amplitude_mask = amplitude_mask
