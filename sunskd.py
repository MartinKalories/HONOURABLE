...
DEFAULT_CSV = "bayesopt_current_nosub_noLR_all_trials.csv"
...
def make_continuous_optimum_dict(optimum_transformed):
    ...
    return {
        ...
        "n_units_dense": optimum_transformed[3],
    }
...
def make_all_kde_variables(df):
    ...
    - all continuous variables are kept in their original numerical values
    ...
    X["n_units_dense"] = df["n_units_dense"].astype(float)
    variable_info["n_units_dense"] = {
        "label": "n_units_dense",
        "type": "continuous",
    }
...
def plot_2d_kde_corner(
    ...
    def tick_labels_for(c):
        ...
        if variable_info[c]["type"] != "continuous":
            return variable_info[c]["ticklabels"]

        if c == "n_units_dense":
            return [f"{t:.0f}" for t in ticks]
