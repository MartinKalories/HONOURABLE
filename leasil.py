...
# Save location
# ------------------------------------------------------------
save_dir = "/home/manav/PL-NN-testdata_forDec2025/"
...
run_prefix = f"bayesopt_{timestamp}_calls{N_CALLS}_init{N_INITIAL_POINTS}"
progress_plot_path = f"{save_dir}{run_prefix}_progress.png"
final_plot_path = f"{save_dir}{run_prefix}_final_progress.png"
...
def save_best_results(best_pdict, best_loss, trial_log, prefix):
    with open(f"{save_dir}{prefix}_best_results.json", "w") as f:
        json.dump(
            make_json_safe({
    ...
    np.savez(
        f"{save_dir}{prefix}_best_results.npz",
        best_objective_val_loss=float(best_loss),
        best_params=np.array([best_pdict], dtype=object),
        all_trials=np.array(trial_log, dtype=object),
    )

    save_trial_log_csv(trial_log, f"{save_dir}{prefix}_all_trials.csv")


def save_skopt_result(res, prefix):
    with open(f"{save_dir}{prefix}_skopt_result.pkl", "wb") as f:
        pickle.dump(res, f)
...
def update_live_plot():
    ...
    fig_live.savefig(progress_plot_path, dpi=300, bbox_inches="tight")
    print("Saved progress plot to:", progress_plot_path)
...
def objective(**params):
    ...
    save_trial_log_csv(trial_log, f"{save_dir}{run_prefix}_all_trials.csv")
    np.savez(
        f"{save_dir}{run_prefix}_all_trials.npz",
        all_trials=np.array(trial_log, dtype=object),
...
def main():
    ...
    if res is not None:
        ...
        fig_live.savefig(final_plot_path, dpi=300, bbox_inches="tight")
        print("Saved final progress plot to:", final_plot_path)
