...
def make_circular_mask(shape, radius_pixels=None):
    """
    Make a circular Boolean mask centred on the image.
    ...
    if radius_pixels is None:
        radius_pixels = min(ny, nx) / 2.0

    y, x = np.ogrid[:ny, :nx]

    r2 = (x - cx) ** 2 + (y - cy) ** 2

    return r2 <= radius_pixels ** 2
def make_assumed_amplitude_wavefront(
    target_phase: np.ndarray,
    radius_pixels: Optional[float] = PUPIL_RADIUS_PIXELS,
):
    """
    Build an assumed complex wavefront from a measured phase image.

    Amplitude is assumed to be:
        1 inside the circular aperture
        0 outside the circular aperture
    """
    target_phase = wrap_phase(target_phase)

    if radius_pixels is None:
        amplitude = np.ones_like(target_phase, dtype=float)
    else:
        aperture_mask = make_circular_mask(
            target_phase.shape,
            radius_pixels=radius_pixels,
        )
        amplitude = aperture_mask.astype(float)

    complex_wavefront = amplitude * np.exp(1j * target_phase)

    return amplitude, complex_wavefront
