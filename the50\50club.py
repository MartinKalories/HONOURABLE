...
# ------------------------------------------------------------
RUN_ID = "bayesopt_current_nosub_noLR_all_trials)_w_50extra"
...
def evaluate_trial(params):
    ...
    try:
        ...
        if not np.isfinite(final_val_loss):
            final_val_loss = 50.0

        return objective_val, final_val_loss
...
def main():
    ...
    try:
        ...
        # These trials update the optimiser using opt.tell()
        # ========================================================
        while len(trial_log) < n_optimisation_trials:
            x = opt.ask()
            ...
            try:
                objective_val, final_val_loss = evaluate_trial(params)

            except Exception as e:
                print("Optimisation trial crashed with params:", params)
                print("Error:", repr(e))
            ...
            # This updates the Bayesian optimiser.
            ...
            print(f"Bayesian trial {len(trial_log)} objective: {objective_val:.8f}")
            print(f"Params: {params}")
            print(f"Running best loss: {min(live_losses):.8f}")
            print("-" * 60)

        # ========================================================
        # Stage 2: Random post-optimisation exploration
        # These trials do NOT update the optimiser.
        # ========================================================
        random_trials_completed = sum(
            trial.get("trial_type") == "random_post"
        ...
        while random_trials_completed < n_random_trials:
            ...
            try:
                ...
                print("Random post-optimisation trial crashed with params:", params)
                print("Error:", repr(e))
                print("Stopping. Rerun the file to resume.")

                save_checkpoint(opt)
                return

            # Deliberately do not call:
            # opt.tell(x_random, objective_val)

            trial_entry = {
                "trial": int(len(trial_log) + 1),
                "trial_type": "random_post",
                "params": make_json_safe(copy.deepcopy(params)),
                "objective_val_loss": float(objective_val),
                "final_val_loss": float(final_val_loss),
            }
            ...
            print(
                f"Random trial {random_trials_completed}/{n_random_trials} "
                f"objective: {objective_val:.8f}"
            )
            print(f"Params: {params}")
            print(f"Running best loss: {min(live_losses):.8f}")
            print("-" * 60)
