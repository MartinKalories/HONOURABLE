...
def make_nearfield_lp_modes(
    ...
    for mode_num in range(n_modes):
        ...
        mode_field = resize_complex_image_to_shape(
            near_field,
            target_shape,
        )

        # Normalise each mode to equal total power
        mode_field = normalise_power(
            mode_field
        )
    ...
    mode_matrix = near_modes.reshape(
        n_modes_actual,
        ny * nx,
    ).T
    ...
    print(
        "Near-field LP mode shape used for fit:",
        (ny, nx),
    )
